...
class DECISION_Tree:
    ...
    def __init__(self, value)->None:
        self.logger = Logger.get_logger('DECISION_Tree')
        self.value = value
    ...
    def chooseBestFeatureToSplit(self,dataSet):
        ...
        #获取dataSet的第i个所有特征
            ...
            self.logger.print("第%d个特征的增益为%.3f" % (i, infoGain))             #打印每个特征的信息增益
...
Returns:
    ...
    def Bool(self,result,myTree):
        self.logger.print(myTree)
        if result == 'yes':
            self.logger.print("放贷")
        if result == 'no':
            self.logger.print("不放贷")


    
 
def decision(x):
    decision = DECISION_Tree(x) 
    dataSet, labels = DECISION_Tree.createDataSet()
    featLabels = []
    myTree = decision.createTree(dataSet, labels, featLabels)

    #print(myTree)
    testVec = [0, 1]     # 测试数据
    result = decision.classify(myTree, featLabels, testVec)
    decision.Bool(result,myTree)
